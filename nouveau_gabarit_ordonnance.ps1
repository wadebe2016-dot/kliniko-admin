# Remplace le gabarit d'impression des ordonnances par une mise en page soignee.
# A executer depuis n'importe ou :  powershell -ExecutionPolicy Bypass -File .\nouveau_gabarit_ordonnance.ps1

$chemin = "C:\Users\wadeb\kliniko-admin\src\Ordonnances.tsx"
if (-not (Test-Path $chemin)) { Write-Host "ECHEC  $chemin introuvable"; exit 1 }
...
if ($debut -lt 0 -or $posFin -lt 0) {
  Write-Host "ECHEC  ancres introuvables (fonction imprimer)"
  exit 1
}
...
Write-Host "OK     gabarit remplace"
Write-Host ("Controle : cadre-sign = " + ([regex]::Matches($s, "cadre-sign")).Count + " occurrences (attendu 3)")
Write-Host ("Taille  : " + (Get-Item $chemin).Length + " octets")
